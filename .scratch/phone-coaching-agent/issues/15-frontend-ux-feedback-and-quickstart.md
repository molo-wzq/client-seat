...
- `MaterialStep`:分析时显示「正在分析转写稿,可能需要 1–2 分钟…」(2026-09-09 验收实测分析最长 90s,文案据此修正);发布时显示「正在发布策略卡…」,保存时显示「正在保存…」。
...
- 四处等待点提示:素材分析/发布、画像接通、对话思考、结果生成、快速开始。分析文案按验收实测修正为「可能需要 1–2 分钟…」。
...
**2026-09-09 智能体验收(真模型活体走查,UI 视觉走查待人工复核)**:

| 验收口径项 | 实测 | 结论 |
| --- | --- | --- |
| 首页一键直接进对话 | `POST /api/quickstart` 0.005s 返回 p01 会话;重复调用幂等不重复入库 | 通过 |
| 发送等待点提示 | 真模型首轮经理回复 26s;「理财经理正在思考…」提示已由 UI 测试断言 | 通过 |
| 分析等待点提示 | 真模型分析 5 轮转写稿实测 **90s**;据此把文案从「约需 10–20 秒」修正为「可能需要 1–2 分钟…」 | 通过(文案已修正) |
| 结束/结果 | finish+result 秒回,mainGoal/策略路径正常返回 | 通过 |
| 发送后客户话立即上屏 | 乐观更新三态(上屏/成功替换/失败回滚)由 `src/ui/CallStep.test.tsx` 覆盖 | 通过(jsdom) |

浏览器内真实点击流(提示的可见性/样式观感)无浏览器驱动工具,待人工打开页面复核;服务端数据已还原,冒烟数据未留在库中。
