...
- `MaterialStep`:分析时显示「正在分析转写稿,约需 10–20 秒…」;发布/保存时显示「正在保存…」。
...
- `CallStep.send()`:发送时先把客户这条话乐观追加到本地 `conversation.turns`(临时轮次号 = 当前最大+1)并清空输入框,再 `await api.sendCustomerTurn`,成功后用返回值替换整体;失败回滚到发送前状态并显示错误。
- 不做流式;不改动域层。
...
- [x] 拆解:三项各定实现方案与范围。
- [ ] BusyHint 组件 + 四步加载提示接入。
- [ ] CallStep 乐观更新(含失败回滚)。
- [ ] 产品缝 quickStart:域层 + InProcess + Http + server 路由。
- [ ] App 快速开始入口。
- [ ] UI 测试与域层测试。
- [ ] CONTEXT.md 补「快速开始」词条(triage 已确认引入该领域词)。
